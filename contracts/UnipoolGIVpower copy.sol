...
contract UnipoolGIVpower is UnipoolTokenDistributor, IERC20MetadataUpgradeable {
    ...
    function lock(uint256 amount, uint256 rounds) external {
        _lock(msg.sender, amount, rounds);
    }
    ...
    function stakeToAndLock(address recipient, uint256 amount, uint256 rounds) external {
        depositToken.safeTransferFrom(msg.sender, address(this), amount);
        depositTokenBalance[recipient] = depositTokenBalance[recipient].add(amount);
        _addBalance(recipient, amount);
        _lock(recipient, amount, rounds);
        emit DepositTokenDeposited(msg.sender, amount);
    }

    function _lock(address recipient, uint256 amount, uint256 rounds) internal {
           if (rounds == 0) {
        ...
        UserLock storage _userLock = userLocks[recipient];

        if (depositTokenBalance[recipient].sub(_userLock.totalAmountLocked) < amount) {
        ...
        if (_gainedPowerAmount > 0) {
            ...
            _addBalance(recipient, _gainedPowerAmount);
        }

        emit TokenLocked(recipient, amount, rounds, _endRound);
    }
}
